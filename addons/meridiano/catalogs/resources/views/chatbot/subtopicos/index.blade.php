...
<div class="flex items-center mb-6">
    <h1 class="flex-1">Subtópicos</h1>
    <a href="{{ cp_route('chatbot.subtopicos.create') }}" class="btn-primary">Crear Nuevo Subtópicos</a>
</div>
...
<div class="card p-2">
    <table class="w-full text-center bg-white">
        ...
        <tbody>
            ...
            <tr>
                <td class="py-2 px-4 border-b border-gray-200">{{ $subtopico->nombre }}</td>
                <td class="py-2 px-4 border-b border-gray-200">{{ $subtopico->descripcion }}</td>
                <td class="py-2 px-4 border-b border-gray-200 max-w-xs w-1/4">
                    <a href="{{ cp_route('chatbot.subtopicos.edit', $subtopico->id) }}" class="btn btn-sm btn-primary"><img src="{{ asset('svg/edit.svg') }}" alt="editar" class="w-5 h-5"></a>
                    <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#deleteModal" data-id="{{ $subtopico->id }}" onclick="showModal({{ $subtopico->id }})"><img src="{{ asset('svg/rubbish-bin.svg') }}" alt="Eliminar" class="w-5 h-5"></button>
                </td>                   
                </tr>
            @endforeach
        </tbody>
    ...
    <div class="w-full">
        <div>
            {{ $subtopicos->links('pagination.tailwind') }}
        </div>
    </div>
...
<script>
    function showModal(id) {
        document.getElementById('deleteForm').action = "{{ cp_route('chatbot.subtopicos.destroy', '') }}/" + id;
