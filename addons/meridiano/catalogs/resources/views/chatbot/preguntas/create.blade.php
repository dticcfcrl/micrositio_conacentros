...
<div class="flex items-center mb-6">
    <h1 class="flex-1">Crear Pregunta</h1>
    <a href="{{ cp_route('chatbot.preguntas') }}" class="btn">Regresar</a>
</div>

<hr><br>
<div class="card p-3">
    <form id="form_pregunta" action="{{ cp_route('chatbot.preguntas.store') }}" method="POST" class="max-w-lg mx-auto bg-white p-8 rounded-lg shadow-md">
        @csrf
        <div class="mb-4">
            <label for="pregunta" class="block text-sm font-medium text-gray-700">Pregunta</label>
            <textarea name="pregunta" id="pregunta" rows="2" class="p-2 mt-1 block w-full rounded-md border border-grey-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required></textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="subtopico_id[]" id="subtopico_id" multiple class="p-2 mt-1 block w-full rounded-md border border-grey-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                @foreach($subtopicos as $subtopico)
                    <option value="{{ $subtopico->id }}">{{ $subtopico->nombre }}</option>
                @endforeach
            </select>
        </div>
