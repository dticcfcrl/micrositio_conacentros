...
@section('content')
    ...
    <h1 class="mb-6 pt-20 text-center text-gray-800">{{ $title == "Activate Account" ? "Activa Cuenta" : "Reinicia Contraseña" }}</h1>
    ...
    <div class="card auth-card mx-auto">
        ...
        <form method="POST" action="{{ $action }}">
            ...
            <div class="mb-8">
                <label for="password-confirm" class="mb-2">{{ __('Confirma contraseña') }}</label>

                <input id="password-confirm" type="password" class="input-text input-text" name="password_confirmation" required>
            </div>

            <button type="submit" class="btn-primary">{{ $title == "Activate Account" ? "Activa Cuenta" : "Reinicia Contraseña" }}</button>
